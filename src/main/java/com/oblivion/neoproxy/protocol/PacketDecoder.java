...
public class PacketDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (!in.isReadable()) {
            LOGGER.trace("[{}] Input buffer not readable.", channelId);
            return;
        }
        ...
        try {
            ...
            LOGGER.trace("[{}] Could not read packet length VarInt, resetting reader index. Error: {}", channelId, e.getMessage());
            in.resetReaderIndex();
            return;
        }

        LOGGER.debug("[{}] Attempting to decode packet. Declared length: {}", channelId, packetLength);
        ...
        if (in.readableBytes() < packetLength) {
            LOGGER.trace("[{}] Not enough readable bytes ({}) for packet length ({}). Resetting reader index.",
        ...
        LOGGER.debug("[{}] Successfully decoded packet. Length: {}, Actual Bytes Read: {}. Passing to next handler.",
        ...
        LOGGER.trace("[{}] Remaining readable bytes in input buffer after decode: {}", channelId, in.readableBytes());
    }
